...
const cmakeMinimumRequiredMajor = 3;
const cmakeMinimumRequiredMinor = 21;
const cmakeMinimumRequired = `${cmakeMinimumRequiredMajor}.${cmakeMinimumRequiredMinor}`;
...
function genCMakePresetsJson(config: Config, buildDir: string): string {
    ...
    const preset = {
        ...
        cmakeMinimumRequired: {
            major: cmakeMinimumRequiredMajor,
            minor: cmakeMinimumRequiredMinor,
            patch: 0,
        },
        configurePresets: [
            {
...
function genProlog(project: Project): string {
    let str = `# machine generated by fibs, don't edit or commit to version control!\n`;
    str += `cmake_minimum_required(VERSION ${cmakeMinimumRequired})\n`;
