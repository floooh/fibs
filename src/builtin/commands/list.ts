...
async function run(project: Project, cmdLineArgs: string[]): Promise<void> {
    ...
    if (args.all || args.configs) {
        ...
        for (const c of sortedConfigs) {
            if (log.verbose()) {
                log.print(`${colors.green(c.name)} <= ${colors.brightBlue(c.importDir)}`);
            } else {
                log.print(c.name);
            }
        }
    ...
    if (args.all || args.imports) {
        for (const i of project.imports().toReversed()) {
            if (imports.isLinked(project, i.name)) {
                log.print(`${colors.green(i.name)}: ${colors.brightBlue(`link => ${i.importDir}`)}`);
            } else {
                log.print(`${colors.green(i.name)}: ${i.url}`);
            }
            for (const mod of i.modules) {
                if (mod.module.help) {
                    mod.module.help(mod.name);
                }
            }
        }
    }
