...
export const CMakeMinimumRequired = `${CMakeMinimumRequiredMajor}.${CMakeMinimumRequiredMinor}`;

const jobFibsRoot = 'jsr:@floooh/fibs';
//const jobFibsRoot = '../fibs/index.ts';   // for debugging

export function genCMakePresetsJson(config: Config, buildDir: string): string {
...
export function genCMakeListsTxt(project: Project, config: Config): string {
    let str = '';
...
function genLinkDirectories(project: Project): string {
    let str = '';
    project.linkDirectories().forEach((item) => str += `link_directories(${compileExpr(item.language, item.buildMode, item.dir)})\n`);
    return str;
}
...
function genJobs(project: Project, config: Config) {
    const allInputs = project.targets().flatMap((t) => t.resolvedJobs.flatMap((j) => j.inputs));
    const allOutputs = project.targets().flatMap((t) => t.resolvedJobs.flatMap((j) => j.outputs));
    if ((allOutputs.length === 0) && (allInputs.length === 0)) {
        return '';
    }
    const uniqueInputs = [...new Set(allInputs)];
    const uniqueOutputs = [...new Set(allOutputs)];
    const outputsPart = `OUTPUT ${uniqueOutputs.join(' ')}`;
